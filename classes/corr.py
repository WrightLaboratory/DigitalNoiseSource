...
class Corr_Data_CRS_LBN:
    def __init__(self,Data_Directory,Gain_Directory,site_class,
                 Data_File_Index=[0,-1],fbounds=[0,1024],vinputs=[0,1,2]):
        
        self.Data_Directory=Data_Directory
        self.Gain_Directory=Gain_Directory
        file_list = ([x for x in os.listdir(self.Data_Directory) if ".lock" not in x])[:-1]
        filelist = sorted(file_list,key=natural_sort_key)
        #filelist = np.sort([x for x in os.listdir(self.Data_Directory) if ".lock" not in x])[:-1]
        self.filenames=filelist[Data_File_Index[0]:Data_File_Index[1]]
        n_files = len(self.filenames)
        ...
        self.t0=fd['index_map']['time'][0]
        self.freq=np.array(fd['index_map']['freq'][flb:fub]) # frequency axis
        self.prod=fd['index_map']['prod'][:] # product axis
        self.n_channels=min(len(vinputs),len(fd['index_map']['prod']))
        print(self.n_channels)
        ## Initialize Visibility and Time data products:
        
        
        self.V=np.zeros((n_files,vis.shape[0],vis.shape[1],vis.shape[2])).astype(complex) #t,nu,prod
        self.t=np.zeros((n_files,vis.shape[0]))
        self.sat=np.zeros((n_files,vis.shape[0],vis.shape[1],vis.shape[2]))
        
        # Empty space for reading in gains...
        ...
        for i,file in enumerate(self.filenames):
            #try:
                print("\r  --> Loading File: {}/{}".format(self.filenames[i],self.filenames[-1]),end="")
                fd_n=h5py.File(self.Data_Directory+file, 'r')
                fdshape = fd['vis'][:,flb:fub,:].shape
                fnshape = fd_n['vis'][:,flb:fub,:].shape
                vis = np.zeros(fdshape).astype(complex)
                vis[:fnshape[0],:fnshape[1],:fnshape[2]]= fd_n['vis'][:,flb:fub,:] # Visibility matrix
                
               
                tm = np.zeros(len(fd['index_map']['time']))
                tm[:len(fd_n['index_map']['time'])]=np.array(fd_n['index_map']['time']) # time axis
...
#                print('\nSkipping file: {}'.format(file))    
        ...
        self.V=self.V.reshape((n_files*vis.shape[0],vis.shape[1],vis.shape[2]))
        self.t=self.t.reshape(n_files*vis.shape[0])
        ...
        for i,file in enumerate(self.filenames):
            try:
                ...
                if ctime=='True':        
                    ...
                    tm=(2.56e-6)*np.array(fd_n['index_map']['time']['fpga_count']) - corrtoff
                if ctime=='False': # instead use irigtime
