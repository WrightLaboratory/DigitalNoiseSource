    ...
    def __init__(self,Data_Directory,Gain_Directory,site_class,Data_File_Index=None,Load_Gains=True,Fix_Gains=False,Apply_Gains=True,Gain_Params=[1.0,24.0],fbounds=[0,1024],use_ctime=False,crossmap=None,esystem=False):
        ...
        for i,file in enumerate(self.filenames[Data_File_Index]):
            try:
                ...
                if not use_ctime:
                    tm = np.array(fd_n['index_map']['time']['irigb_time'])  # default time axis
                    # DNS overrides GBO
                    if 'DNS' in site_class.name or 'WLC' in site_class.name:
                        tm = (2.56e-6) * np.array(fd_n['index_map']['time']['fpga_count'])
                    elif 'GBO' in site_class.name:
                        tm = (2.56e-6) * np.array(fd_n['index_map']['time']['fpga_count'])

                if use_ctime:
                    tm = np.array(fd_n['index_map']['time']['ctime'])  # default time axis
                    if 'DNS' in site_class.name or 'WLC' in site_class.name:
                        tm = (2.56e-6) * np.array(fd_n['index_map']['time']['fpga_count'])
