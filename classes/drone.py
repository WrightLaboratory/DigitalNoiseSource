...
class Drone_Data:
    def __init__(self, Drone_Directory, FLYTAG, site_class, tlb=0, tub=-1, ignore_rtk=False, skip_rows=False):
        ...
        #######################################################
        ## Calculate per-dish polar coordinates
        #######################################################
        print("  --> generating dish and receiver line of sight coordinates.")
        self.xyz_per_dish = np.zeros((self.n_dishes, len(self.t_index), 3))
        self.rpt_r_per_dish = np.zeros((self.n_dishes, len(self.t_index), 3))
        self.rpt_t_per_dish = np.zeros((self.n_dishes, len(self.t_index), 3))
