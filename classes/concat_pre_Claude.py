...
class CONCAT:
    def __init__(self,CORRDATCLASS,DRONEDATCLASS,config_directory="/hirax/GBO_Analysis_Outputs/concat_config_files/",output_directory='/hirax/GBO_Analysis_Outputs/',load_yaml=True,traceback=True,save_traceback=True,t_drone_offset=0,t_bounds=None):
        ...
        ## Apply drone timing correction BEFORE selecting correlator data window:
        drone_t_min = DRONEDATCLASS.t_arr_datetime[0] + datetime.timedelta(seconds=t_drone_offset)
        drone_t_max = DRONEDATCLASS.t_arr_datetime[-1] + datetime.timedelta(seconds=t_drone_offset)
        ...
        print("Created drone_pitch_interp")
        ...
        print("Filled drone_pitch_interp")
        self.tstep=1e-9*np.nanmedian(np.diff(self.t))
        if self.t_bounds is not None:
            i0, i1 = self.t_bounds

            # Time-dependent correlator data
            self.t_arr_datetime = self.t_arr_datetime[i0:i1]
            self.t_index = self.t_index[i0:i1]
            self.t = self.t[i0:i1]

            self.V = self.V[i0:i1, :, :]

            if hasattr(self, "V_cross"):
                self.V_cross = self.V_cross[i0:i1, :, :]

            # Time-dependent interpolated drone data
            self.drone_llh_interp = self.drone_llh_interp[i0:i1, :]
            self.drone_xyz_LC_interp = self.drone_xyz_LC_interp[i0:i1, :]
            self.drone_rpt_interp = self.drone_rpt_interp[i0:i1, :]

            self.drone_yaw_interp = self.drone_yaw_interp[i0:i1]
            self.drone_pitch_interp = self.drone_pitch_interp[i0:i1]

            # Arrays with shape (dish, time, xyz)
            self.drone_xyz_per_dish_interp = self.drone_xyz_per_dish_interp[:, i0:i1, :]
            self.drone_rpt_r_per_dish_interp = self.drone_rpt_r_per_dish_interp[:, i0:i1, :]
            self.drone_rpt_t_per_dish_interp = self.drone_rpt_t_per_dish_interp[:, i0:i1, :]
    ...
    def Synchronization_Function(self,inputcorr,inputdrone,coarse_params=[-10.0,10.0,0.2],fine_params=[-0.5,0.5,0.01],chans=np.array([0,1]),freqs=np.arange(100,1024,150),FMB_coordbounds=[30.0,30.0,150.0],FMB_ampbound=0.999,t_bounds=None):
        if self.traceback==True:
        ...
        if hasattr(self,"t_delta_dji")==False:
            ...
            for i, ttry in enumerate(tqdm(t_coarse, desc="Coarse search")):
                ...
                tempconcat=concat.CONCAT(
                    CORRDATCLASS=inputcorr,
                    DRONEDATCLASS=tempdrone,
                    load_yaml=False,
                    traceback=False,
            ...
            if self.traceback==True:
                ...
                for i in range(len(chans)):
                    ...
                    ax.axvline(
                        t_coarse[int(np.nanmedian(GPRmax[i]))],
                        c='r',
                        label='median t = {:.2f}'.format(t_coarse[int(np.nanmedian(GPRmax[i]))])
                    )
                ...
                for i in range(len(chans)):
                    ...
                    ax.axvline(
                        t_fine[int(np.nanmedian(GPRmax_fine[i]))],
                        c='r',
                        label='median t = {:.2f}'.format(t_fine[int(np.nanmedian(GPRmax_fine[i]))])
                    )
                    ax.set_title('Channel {} Fine Offset Correlation'.format(chans[i]))
                    ax.set_xlabel('$\Delta$t $[sec]$')
                    ax.set_ylabel('Pearson R Value')
                    ax.legend(loc=1, fontsize='small')
                tight_layout()
        ...
        print("Has yaw:", hasattr(tempconcat, "drone_yaw_interp"))
        print("Has pitch:", hasattr(tempconcat, "drone_pitch_interp"))
    ...
    def Main_Beam_Fitting(self,fit_param_directory='/hirax/GBO_Analysis_Outputs/main_beam_fits/',freqs=np.arange(1024),theta_solve=False,FMB_ampbound=0.999,coordbounds=[80.0,80.0,150.0],Vargs='None'):
        if self.traceback==True:
